...
input_mapper = TokenMapper(input_utterances)
target_mapper = TokenMapper(target_utterances)
...
num_encoder_tokens = len(input_mapper.tok2num)
num_decoder_tokens = len(target_mapper.tok2num)
...
for i, (input_utterance, target_utterance) in enumerate(zip(input_utterances, target_utterances)):
    for j, token in enumerate(input_utterance):
        ...
        encoder_input_data[i][j] = input_mapper.tok2num[token]

    for k, token in enumerate(target_utterance):
        # One-hot encoding for decoder input data.
        decoder_input_data[i][j] = input_mapper.tok2num[token]
        ...
        if k > 0:
            ...
            # no longer included.
            decoder_target_data[i, k-1, target_mapper.tok2num[token]] = 1
